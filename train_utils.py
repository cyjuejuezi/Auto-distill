...
def train_and_evaluate(args, run, tokenizer, tokenized_datasets, compute_metrics):
    ...
    trainer = Seq2SeqTrainer(**trainer_kwargs) if args.model_type == 'standard' else TaskPrefixTrainer(**trainer_kwargs)

    # Start training
    trainer.train()

    # After training, log desired metrics to CSV
    csv_file_path = 'loss_accuracy.csv'
    # After training, log desired metrics to CSV
    with open(csv_file_path, 'w', newline='') as file:
        ...
        for log in trainer.state.log_history:
            ...
            if 'eval_loss' in log and 'epoch' in log:  # Validation log
                validation_loss = log['eval_loss']
                validation_accuracy = log.get('eval_accuracy', "N/A")
                writer.writerow([epoch, training_loss, validation_loss, validation_accuracy])
