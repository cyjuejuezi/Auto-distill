...
#
# # 加载分词器和模型
# tokenizer = AutoTokenizer.from_pretrained("/root/CY/CodeSearchNet/gpt2_java")
# model = AutoModelForCausalLM.from_pretrained("/root/CY/CodeSearchNet/gpt2_java")
#
# # 创建一个文本生成pipeline
...
#
# # 选择几个测试样本
# test_samples = dataset.select(range(5))  # 选取测试集前5个样本
#
# # 构造输入
...
#
# # 定义一个函数来生成文本
# def generate_responses(text_examples, max_length=512):
#     return text_generator(text_examples, max_length=max_length, num_return_sequences=1)
#
# # 计算字符串相似度得分的函数
...
#
# # 选择几个测试样本
# test_samples = dataset['func_name'][:5]  # 取测试集前5个样本
#
# # 生成回应
# results = generate_responses(test_samples)
#
# # 计算平均得分
# scores = []
# for idx, result in enumerate(results):
#     generated_text = result[0]['generated_text']  # 从字典中提取生成的文本
#     original_string = test_samples[idx]
#     score = similarity_score(original_string, generated_text)
#     scores.append(score)
#
...
#
# # 选择几个测试样本
# test_samples = dataset.select(range(10))  # 选取测试集前10个样本
#
# # 构造输入
# prompts = build_prompts(test_samples['func_name'])
#
...
# 选择几个测试样本
test_samples = dataset.select(range(10))  # 选取测试集前5个样本，注意更正之前的注释
